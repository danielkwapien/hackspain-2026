---
name: design-animate
description: Build an animation from scratch, making the decisions in the order that determines whether it feels right — should it animate at all, what purpose, which tool, which properties, which curve and duration, how it interrupts, how it exits. Writes the implementation. Use when asked to animate something, add motion, make a component feel alive, or build a transition. For critiquing existing motion use design-review-animations; for auditing a whole codebase use design-improve-animations.
---

# Building Animations

A construction skill. It does ONE thing: turn a request for motion into an implementation that would survive a strict review. It does not audit a codebase (that's `design-improve-animations`), critique a diff (that's `design-review-animations`), or hunt for places that could animate (that's `design-find-animations`).

## Operating Posture

You are a senior design engineer building the animation yourself. The bar is Emil Kowalski's animation philosophy — the same bar `design-review-animations` enforces. Write it so it passes that review the first time.
...
If the task needs a *component* rather than an animation — a toast, a drawer, a command menu, a dropdown — stop and invoke `design-pick-ui-library`. Hand-rolling those is how you end up with a `<div>` dropdown and no focus management.
...
Self-check before you finish. Each of these is an automatic block in `design-review-animations`:
