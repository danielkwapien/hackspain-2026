...
export async function readDetail(client: MotherDuckClient, company: CompanyListItem): Promise<CompanyDetail> {
  ...
  const banking = await client.query(`SELECT p.product_id,p.label,p.type,p.bank_name,p.service,p.currency,p.created_at::varchar created_at,
    ...
    LEFT JOIN (SELECT product_id,company_id,max(date) date FROM balances WHERE date <= (SELECT cutoff_date FROM score_exports LIMIT 1) GROUP BY ALL) b USING(product_id,company_id) WHERE p.company_id=? ORDER BY product_id`, bank, id);
  ...
  const balances = await client.query(`SELECT product_id,date::varchar date,balance::double balance,available::double available,liquidity::double liquidity,countable::double countable FROM balances WHERE company_id=? AND date <= (SELECT cutoff_date FROM score_exports LIMIT 1) ORDER BY date,product_id`, balance, id);
  ...
  const monthly = await client.query(`SELECT strftime(t.date,'%Y-%m') AS month,coalesce(p.currency,'UNKNOWN') currency,count(*) FILTER(WHERE t.status='booked')::integer n_tx,
    ...
    strftime(t.date,'%Y-%m')=strftime((SELECT cutoff_date FROM score_exports LIMIT 1),'%Y-%m') AS partial
    FROM transactions t LEFT JOIN ${products} p USING(product_id,company_id)
    WHERE t.company_id=? AND t.status IN ('booked','pending') AND t.date <= (SELECT cutoff_date FROM score_exports LIMIT 1)
  ...
  const monthlyInvoices = await client.query(`SELECT strftime(issuance_date,'%Y-%m') AS month,currency,count(*)::integer n,
    ...
    AND issuance_date <= (SELECT cutoff_date FROM score_exports LIMIT 1) GROUP BY 1,2 ORDER BY 1,2`, invoice, id);
  return {
