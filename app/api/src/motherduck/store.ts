...
export async function loadMotherDuckStore(client: MotherDuckClient): Promise<V2Store> {
  ...
  return {
    dir: "md:hackspain_2026", manifest: {
      ...
      notes: ["Datos sintéticos oficiales del reto, consultados en MotherDuck.", "El inventario v1 solo describe el dato de origen: el score, su historia y las predicciones los sirve el motor real en /api/v2.", "La cobertura cuenta todos los meses naturales de la ventana global, incluidos el inicial y el mes parcial del corte.", "Los recuentos de transacciones y facturas incluyen todo el fichero fuente; los meses de actividad y los importes de movimientos usan solo contabilizados (booked) hasta el corte; pendientes se cuentan aparte.", "La caja observada suma únicamente productos bancarios por moneda; la deuda se expone por producto y no reduce ese total.", "Los importes se presentan por moneda; no hay conversión FX implícita."],
