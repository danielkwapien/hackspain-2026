...
export async function loadCoverage(client: MotherDuckClient, companies: CompanyRow[]) {
  const observation = await client.query(`SELECT min(t.date)::date::varchar AS "start",
    e.cutoff_date::varchar AS "end",
    (date_diff('month',min(t.date),e.cutoff_date)+1)::integer periods_total
    FROM transactions t CROSS JOIN score_exports e WHERE t.date <= e.cutoff_date GROUP BY e.cutoff_date`,
  ...
  SELECT c.company_id,
    ...
    (SELECT count(DISTINCT b.product_id)::integer FROM balances b JOIN banking_products p USING(product_id,company_id) WHERE b.company_id=c.company_id AND b.date <= (SELECT cutoff_date FROM score_exports LIMIT 1)) n_products,
