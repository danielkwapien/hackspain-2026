...
|---|---|---|---|
| Fondo de página | `body #181613` (negro cálido), `html #000`. En el tablero **no hay orbe**: `canvas.spotlightCursor` fijo con `z-index: -1` y alfa 0 en todos los píxeles; el foco difuminado cálido solo se pinta en la pantalla de login y sigue al cursor (canvas, no CSS). | `--bg: var(--navy-950)` = `#070b1f`, plano. | Navy **más oscuro**: `--navy-1000` como fondo (candidatos medidos `#04071a` · `#030614` · `#02040e`; los tres mantienen AA: gris secundario ≥ 6,7:1, rojo ≥ 5,7:1, verde ≥ 9:1). El orbe se trae del login a la página como capa CSS pura (`radial-gradient` + `blur`, deriva lenta, `transform`/`opacity`, apagado bajo `prefers-reduced-motion`). Tres variantes en `/prototypes/background` para que Alfonso elija. |
